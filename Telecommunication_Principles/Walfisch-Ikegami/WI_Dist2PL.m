function PL = Q1_WI_Dist2PL(d,f,City)
    ...
    % distance must be greater than 20m.
    ...
    for i = 1:length(d)
        if (h_TX > h_ROOF)
            k_a(i) = 54;
        elseif ((d(i) >= 0.5) && (h_TX <= h_ROOF))
            k_a(i) = 54 - 0.8*h_TX_Delta;
        else 
            k_a(i) = 54 - 0.8*h_TX_Delta*(d(i)/0.5);
        end
    end
    ...
    if (CityVar == 0) % LOS
        ...
        L_los = 42.6 + 26*log10(d) + 20*log10(f);
        PL = L_los; % return result

    elseif (CityVar == 1) % NLOS
        L_0 = 32.44 + 20*log10(d) + 20*log10(f);
        L_rts = -16.9 - 10*log10(w) + 10*log10(f) + ...
            20*log10(h_RX) + L_ori;
        ...
        if ((L_rts + L_msd) < 0)
            ...
            L_nlos = L_0 + L_rts + L_msd;
        end
        PL = L_nlos; % return result
